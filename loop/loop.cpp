...
Loop::~Loop() {
    ...
    int status = uv_loop_close(uv_default_loop());
    logger.info("libuv status: %d", status);
}
...
int Loop::onMessageComplete(http_parser *parser) {
    ...
    if (runType == RT_Sync) {
        ...
        std::string uuid = UUID::create();
        logger.debug("Launching async job with uuid %s", uuid.c_str());
        ...
        // Retrieve the result of a delayed operation
        std::string token = request->params["token"].as<std::string>();
        Reactor::i().placeCallback(token, [request](int code, JSON::Value& result) {
            writeResponse(code, request, result);
        });
    }
...
void Loop::serverOnConnect(uv_stream_t *s, int status) {
    ...
    if (uv_accept(s, (uv_stream_t *) client) == 0) {
        uv_read_start((uv_stream_t *) client,
                      // Alloc memory for a new connection
